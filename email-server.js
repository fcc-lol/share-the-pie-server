...
async function saveToDatabase(record) {
  ...
  const client = new MongoClient(
    `mongodb://${username}:${password}@localhost/?authMechanism=DEFAULT`
  );
  ...
  const completion = await openai.chat.completions.create({
    messages: [
      ...
      },
      {
        role: "user",
        content:
          "when did this transaction occur? what was the merchant's name? create a list of the items, excluding items that have zero price or no price or blank price, and show the grand total amount and tax and tip that is shown on this receipt, where the subtotal, tax, and tip needs to add up to the grand total"
      }
    ],
...
server.listen(process.env.SERVER_SMTP_PORT, process.env.SERVER_IP, () => {
  console.log(
    `Server started on ${process.env.SERVER_IP} at port ${process.env.SERVER_SMTP_PORT}\n\nListening for emails sent to *@${process.env.DOMAIN_NAME}`
  );
});
